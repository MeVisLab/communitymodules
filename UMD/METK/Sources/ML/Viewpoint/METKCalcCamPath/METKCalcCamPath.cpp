...
ML_CLASS_SOURCE(METKCalcCamPath, ObjMgrClient);


const char* METKCalcCamPath::prefRegionTypeStrings[2] = { "Vector region", "Point region" };
const char* METKCalcCamPath::sphereModeStrings[2] = { "Grid Sphere", "Full Sphere" };
const char* METKCalcCamPath::apexAngleStrings[4] = {"20", "30", "40", "50" };


//----------------------------------------------------------------------------------
//! Constructor / Destructor
//----------------------------------------------------------------------------------
METKCalcCamPath::METKCalcCamPath() : inherited(0,0,ObjMgrClient::EVENTS_SELECTED)
{
	ML_TRACE_IN("METKCalcCamPath::METKCalcCamPath()");
	...
	fldCampathApexAngle = getFieldContainer()->addFloat("campathApexAngle");
	fldCampathApexAngle->setFloatValue(20.0);
    ...
    oReceiver->messageFld->setStringValue("solverCamPath");
    ...
    timerSensor = new SoTimerSensor((SoSensorCB*)METKCalcCamPath::timerEvent, this);
...
METKCalcCamPath::~METKCalcCamPath() {}


void METKCalcCamPath::activateAttachments()
{
	clearAcceptedObjectIDs();
	clearAcceptedInfoLayerNames();
	addAcceptedObjectID("*");
...
//----------------------------------------------------------------------------------
void METKCalcCamPath::handleNotification (Field *field)
{
	if (field == _init)
	{
		init();
		...
		SbVec3f posold;
		myObjMgr->getObjAttributeVec3f(_viewerName->getStringValue(), LAY_VIEWER_CAMERA, INF_VIEWER_CAMERA_POSITION, posold);
		for(int i=1;i<=4;i++){
			m_calcVis.clearStack(i);
		}
		//m_calcVis.calc();
		...
		m_calcVis.copyFieldToStack(SUM_FIELD,3);
		m_calcVis.getStackField(3)->normalize();
		m_calcVis.getStackField(4)->fillWith(0.15);
		m_calcVis.addStackFields(1,2,1);
		m_calcVis.getStackField(1)->normalize();
		SbVec3f posnew=Cam->getPosition();
		int fposold=m_calcVis.getFieldPos(posold[0],posold[1],posold[2]);
		int fposnew=m_calcVis.getFieldPos(posnew[0],posnew[1],posnew[2]);
		int factpos=fposold;
		SbVec3f actpos=posold;
		std::list<int> visited;
		visited.push_back(factpos);
		m_calcVis.getStackField(1)->setValueAt(fposnew,1.0);
		int counter=0;
		SbVec3f direction;
		int temppos;
		while(counter<1000 && factpos!=fposnew){
			direction=posnew-actpos;
			temppos=factpos;
			factpos=m_calcVis.findNeighborPointDirection(factpos,direction,visited,posnew);
			if(factpos==-1 || temppos==factpos)
				break;
			m_calcVis.getSpherePos(factpos,actpos[0],actpos[1],actpos[2]);
			visited.push_back(factpos);
			counter++;
		}
		std::list<int>::iterator it=visited.begin();
		myObjMgr->setObjAttribute("CameraSolver","Camera","Count",new int(visited.size()),omINFOTYPE_INT32,true,false);
		int listPosition=0;
		m_calcVis.getStackField(4)->setValueAt(fposold,0.0);
		m_calcVis.getStackField(4)->setValueAt(fposnew,1.0);
		setCamPosition(4,true);
		while(it!=visited.end()){
			m_calcVis.getStackField(4)->setValueAt(*it,0.8);
			m_calcVis.getSpherePos(*it,actpos[0],actpos[1],actpos[2]);
			std::ostringstream osPosition;
			osPosition << "Position" << listPosition;
			std::ostringstream osOrientation;
			osOrientation << "Orientation" << listPosition;
			//std::cout << osPosition.str() << ";" << actpos[0] << "(" << v3[0] << ")" << ";" << actpos[1] << "(" << v3[1] << ")" << ";" << actpos[2] << "(" << v3[2] << ")" << std::endl;
			myObjMgr->setObjAttribute("CameraSolver","Camera",osPosition.str(),new vec3(actpos[0],actpos[1],actpos[2]),omINFOTYPE_VEC3,true,false);
			//myObjMgr->setObjAttribute("CameraSolver","Camera",osOrientation.str(),new vec4(_orient->getVec4fValue()),omINFOTYPE_VEC4,true,false);
			it++;
			listPosition++;
		}
		kDebug::Debug("Update Camera data in CameraSolver object",kDebug::DL_NONE);
	...
	else if (field == fldCampathApexAngle){
		m_calcVis.setApexAngle(fldCampathApexAngle->getFloatValue());
	}
...
void METKCalcCamPath::handleObjMgrNotification()
{	
...
void METKCalcCamPath::setCamPosition(const int stackOrFieldNr, bool isStackNr)
{
	float fX,fY,fZ;
	if (!isStackNr)
...
void METKCalcCamPath::timerEvent(void* data, SoDataSensor* a)
{	
	METKCalcCamPath* caller = (METKCalcCamPath*)data;//->render(a);    
    caller->timerSensor->unschedule();    
    caller->oReceiver->doneFld->notify();
}


void METKCalcCamPath::updateObjectMgr()
{
...
void METKCalcCamPath::writeCamToObjMgr()
{
...
void METKCalcCamPath::enableVisibleObjects()
{
...
const float METKCalcCamPath::getImportance(const std::string* name)
{
	omObjectContainer* oc = getObjContainer();
   if (oc) {
      if ((*oc).exists(*name)) {
...
void METKCalcCamPath::init()
{
	kDebug::Debug("init METKCalcCamPath",kDebug::DL_HIGH);
	string sPath;	
...
void METKCalcCamPath::calcMultiple(bool setCamera)
{
