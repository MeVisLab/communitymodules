...
{
	width = abs(range.x2 - range.x1 + 1);
	height = abs(range.y2 - range.y1 + 1);
	data.resize(width * height);
	color_mode = set_color_mode;
}
...
{
	ifstream file;
	unsigned long ifd_offset, parameter_offset, height_strip = 0;
	unsigned char buffer[12];
	unsigned short ifd_tag, ifd_count, strip_count = 0;
	valarray<unsigned long> strip_offset;
	int i, j, x, y, y_strip, offset_row;
	bool endian;
	...
	for(j = 0; j < ifd_count; j++){
		...
		switch(ifd_tag){
			...
			case 0x0111: // TIFF strip offset(s)
				...
				strip_count = (unsigned short)byte_long(&buffer[4], endian);
				if(strip_count == 1){
...
//				cout << "Tag " << ifd_tag << " is ignored" << endl;
	...
	for(i = 0; i < strip_count; i++){
		...
		for(y_strip = 0; (unsigned int)y_strip < height_strip; y_strip++){
	...
	for(y = height - 1; y >= 0; y--){
		...
		for(x = 0; (unsigned int)x < width; x++){
			if(color_mode == GRAY_8BPP){
				...
				buffer[0] = (unsigned char)(data[offset_row + x] & 0xff); // Red
				buffer[1] = (unsigned char)((data[offset_row + x] & 0xff00) >> 8); // Green
				buffer[2] = (unsigned char)((data[offset_row + x] & 0xff0000) >> 16); // Brue
	...
	for(y = 0; y < height; y++){
		...
		for(x = 0; x < width; x++){
			if(color_mode == GRAY_8BPP){
				...
				buffer[0] = (unsigned char)((data[offset_row + x] & 0xff0000) >> 16); // Blue
				buffer[1] = (unsigned char)((data[offset_row + x] & 0xff00) >> 8); // Green
				buffer[2] = (unsigned char)(data[offset_row + x] & 0xff); // Red
				file.write((char *)buffer, 3);
			}
		}
		if(fill_dword)
...
int image_basic::get(int x, int y) const
{
...
{
	if(point.x >= 0 && (unsigned int)point.x < width && point.y >= 0 && (unsigned int)point.y < height)
		return fast_get(point);
	else
		return 0;
}

void image_basic::set(int x, int y, int value)
{
	if(x >= 0 && (unsigned int)x < width && y >= 0 && (unsigned int)y < height)
		fast_set(x, y, value);
}
...
int image_basic::clip_rect(rect_range &range) const
{
	...
	if((unsigned int)range.x1 >= width) range.x1 = width - 1;
	if(range.x2 < 0) range.x2 = 0;
	if((unsigned int)range.x2 >= width) range.x2 = width - 1;
	...
	if((unsigned int)range.y1 >= height) range.y1 = height - 1;
	if(range.y2 < 0) range.y2 = 0;
	if((unsigned int)range.y2 >= height) range.y2 = height - 1;
...
void image_basic::binarize(const image_basic &image_source, const rect_range &range_source, int threshold, int value_positive, int value_negative)
{
	...
	if((unsigned int)x_range >= width)
		x_range = width - 1;
	y_range = range_source.y2 - range_source.y1;
	if((unsigned int)y_range >= height)
		y_range = height - 1;
	for(y = 0; y <= y_range ;y++){
		for(x = 0; x <= x_range; x++){
...
{
	for(int i = 1; (unsigned int)i < points.size(); i++){
...
{
	int count = 0, x_range, y_range, i, count_outer;
...
void image_basic::scale(int minimum, int maximum)
{
	int x, y, row_offset, value, top_value = 0, range;
...
void image_basic::line_draw(const point_2d &p1, const point_2d &p2, const std::valarray<int> &line)
{
	...
	if(p1.x < p2.x && p1.y == p2.y){
		for(i = 0; (unsigned int)i < line.size(); i++)
...
rect_range::rect_range(int ax1, int ax2, int ay1, int ay2)
{
	x1 = ax1;
	x2 = ax2;
	y1 = ay1;
...
point_2d::point_2d(int ax, int ay)
{
	x = ax;
	y = ay;
}
...
void point_2d::set(int xa, int ya)
{
	x = xa;
	y = ya;
}
void point_2d::get(int &xa, int &ya) const
{
	xa = x;
	ya = y;
}
...
void matrix_2d::forward(point_2d &p) const 
{
	...
	p.x = (int)(matrix_forward[0] * temp.x + matrix_forward[1] * temp.y + matrix_forward[2]);
	p.y = (int)(matrix_forward[3] * temp.x + matrix_forward[4] * temp.y + matrix_forward[5]);
}
void matrix_2d::reverse(point_2d &p) const
{
	point_2d temp(p);

	p.x = (int)(matrix_reverse[0] * temp.x + matrix_reverse[1] * temp.y + matrix_reverse[2]);
	p.y = (int)(matrix_reverse[3] * temp.x + matrix_reverse[4] * temp.y + matrix_reverse[5]);
}
