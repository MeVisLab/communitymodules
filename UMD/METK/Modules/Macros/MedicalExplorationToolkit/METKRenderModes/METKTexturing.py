...
def handleAttributeModifiedEvent():
   ...
   if info == INF_TEXTURIZE or info == INF_VISIBLE:
      if _cls_info.existInfo(object, layer, INF_TEXTURIZE) and _cls_info.getBool(object, layer, INF_TEXTURIZE) == True:
         addContainer(object)
   pass
...
def handleLoadedEvent():
   ...
   while _cls_iter.success():
      if ctx.field("creationTime").stringValue() == "OnLoad" or (_cls_info.existInfo(_cls_iter.object(), LAY_APPEARANCE, INF_TEXTURIZE) and _cls_info.getBool(_cls_iter.object(), LAY_APPEARANCE, INF_TEXTURIZE) == True):
...
def handleCleanupEvent():
   _cls_info.typedSet(OBJ_COMMUNICATION, LAY_STATUS, INF_STATUSMSG, "Cleaning up METKTexturing...", INFOTYPE_MESSAGE)
   _cls_info.notify()
   ctx.log("Clean METKTexturing")
...
def deleteModules():
   ...
   ctx.log("METKTexturing is clean")
...
def addContainer(objectID):
   ...
   if _containerList.count(_cls_info.adjustObjectID(objectID)) == 0:
      ...
      ctx.log("Add Texture Container: " + objectID)
      container = ctx.addModule("TextureContainer")
