...
def handleAttributeModifiedEvent():
   ...
   if info == INF_SILHOUETTE or info == INF_VISIBLE:
      if _cls_info.existInfo(object, layer, INF_SILHOUETTE) and _cls_info.getBool(object, layer, INF_SILHOUETTE) == True:
         addContainer(object)
   pass
...
def handleLoadedEvent():
   ...
   while _cls_iter.success():
      if ctx.field("creationTime").stringValue() == "OnLoad" or (_cls_info.existInfo(_cls_iter.object(), LAY_APPEARANCE, INF_SILHOUETTE) and _cls_info.getBool(_cls_iter.object(), LAY_APPEARANCE, INF_SILHOUETTE) == True):
...
def handleCleanupEvent():
   _cls_info.typedSet(OBJ_COMMUNICATION, LAY_STATUS, INF_STATUSMSG, "Cleaning up METKSilhouette...", INFOTYPE_MESSAGE)
   _cls_info.notify()
   ctx.log("Clean METKSilhouette")
...
def deleteModules():
   ...
   ctx.log("METKSilhouette is clean")
...
def addContainer(objectID):
   ...
   if _containerList.count(_cls_info.adjustObjectID(objectID)) == 0:
      ...
      #ctx.log("Add Silhouette Container: " + objectID)
      container = ctx.addModule("SilhouetteContainer")
      ...
      container.field("visibleOn").setStringValue(ctx.field("visibleOn").value)
...
def modeChanged(field = 0):
   global _containerList
   for module in _containerList:
      ctx.module(module).field("mode").setStringValue(ctx.field("mode").value)
   pass