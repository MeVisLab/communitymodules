...
private:
  ...
  CountVector _symbolCount[U3DContext_StaticFull];     // An array of vectors that stores the number of occurrences of each symbol for each dynamic context.
  CountVector _cumulativeCount[U3DContext_StaticFull]; // An array of vectors that stores the cumulative frequency of each symbol in each context. The value is the number of occurences of a symbol and every symbol with a larger value.
