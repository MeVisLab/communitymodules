//----------------------------------------------------------------------------------
//! This header files contains some PRC constants and definitinos from Adobe's documentation
...
#ifndef _PRC_Defines_H
#define _PRC_Defines_H
...
// PRC Version
#define   PRC_VERSION     (8137)                     // For Adobe Reader 9 or later
#define   CHUNK_SIZE      (1024)
#define   MLuint32_MAX    ( (MLuint32)-1 )

// Base types for serialization
...
#define PRC_TYPE_Unknown  ( MLuint32_MAX )
...
#define PRC_TYPE_ASM_Filter                      ( PRC_TYPE_ASM + 20 ) // Basic construct for filter.
...
#define PRC_PRODUCT_FLAG_DEFAULT        0x0001         // The product occurrence is the default container, configuration, or view. 
#define PRC_PRODUCT_FLAG_INTERNAL       0x0002         // The product occurrence is internal. This flag is used only when the product occurrence has no father. 
#define PRC_PRODUCT_FLAG_CONTAINER      0x0004         // The product occurrence is a container. 
#define PRC_PRODUCT_FLAG_CONFIG         0x0008         // The product occurrence is a configuration. 
#define PRC_PRODUCT_FLAG_VIEW           0x0010         // The product occurrence is a view. 

#define PRC_TRANSFORMATION_Identity         0x00        // Current transformation is an identity. 
#define PRC_TRANSFORMATION_Translate        0x01        // Current transformation has a translation. 
#define PRC_TRANSFORMATION_Rotate           0x02        // Current transformation has a rotation. 
#define PRC_TRANSFORMATION_Mirror           0x04        // Current transformation has a mirror. 
#define PRC_TRANSFORMATION_Scale            0x08        // Current transformation has a uniform scale. 
#define PRC_TRANSFORMATION_NonUniformScale  0x10        // Current transformation has a non-uniform scale. 
#define PRC_TRANSFORMATION_NonOrtho         0x20        // Current transformation is not orthogonal. 
#define PRC_TRANSFORMATION_Homogeneous      0x40        // Current transformation has homogeneous values. 

#define PRC_FACETESSDATA_Polyface                          0x0001        // Not used.
#define PRC_FACETESSDATA_Triangle                          0x0002        // Simple triangle: described with 6 indices (normal,point,normal,point,normal,point). 
#define PRC_FACETESSDATA_TriangleFan                       0x0004        // Triangle fan: described with 2*n indices (normal,point,... ,normal,point). 
#define PRC_FACETESSDATA_TriangleStripe                    0x0008        // Triangle stripe: described with 2*n indices (normal,point,... ,normal,point). 
#define PRC_FACETESSDATA_PolyfaceOneNormal                 0x0010        // Not used .
#define PRC_FACETESSDATA_TriangleOneNormal                 0x0020        // Simple triangle with one normal: described with 4 indices (normal,point,point,point). 
#define PRC_FACETESSDATA_TriangleFanOneNormal              0x0040        // Triangle fan with one normal.
#define PRC_FACETESSDATA_TriangleStripeOneNormal           0x0080        // Triangle stripe with one normal.
#define PRC_FACETESSDATA_PolyfaceTextured                  0x0100        // Not used.
#define PRC_FACETESSDATA_TriangleTextured                  0x0200        // Simple triangle with texture.
#define PRC_FACETESSDATA_TriangleFanTextured               0x0400        // Triangle fan with texture.
#define PRC_FACETESSDATA_TriangleStripeTextured            0x0800        // Triangle stripe with texture.
#define PRC_FACETESSDATA_PolyfaceOneNormalTextured         0x1000        // Not used.
#define PRC_FACETESSDATA_TriangleOneNormalTextured         0x2000        // Simple triangle with one normal and texture.
#define PRC_FACETESSDATA_TriangleFanOneNormalTextured      0x4000        // Triangle fan with one normal and texture.
#define PRC_FACETESSDATA_TriangleStripeOneNormalTextured   0x8000        // Triangle stripe with one normal and texture.
#define PRC_FACETESSDATA_NORMAL_Single                      0x40000000    // If this flag is set, the corresponding OneNormal entity (see PRC tessellation types for FaceTessData) is planar and only one normal is defined for the entity. Otherwise, one normal per point is defined. 
...
enum PRC_ModellerAttributeType 
{
  PRC_ModellerAttributeType_Null   = 0,
  PRC_ModellerAttributeType_Int    = 1,
  PRC_ModellerAttributeType_Real   = 2,
  PRC_ModellerAttributeType_Time   = 3,
  PRC_ModellerAttributeType_String = 4
};

enum PRC_PictureDataFormat 
{
  PRC_PictureDataFormat_PNG,
  PRC_PictureDataFormat_JPG,
  PRC_PictureDataFormat_Bitmap_RGB,
  PRC_PictureDataFormat_Bitmap_RGBAlpha,
  PRC_PictureDataFormat_Bitmap_Grey,
  PRC_PictureDataFormat_Bitmap_GreyAlpha
};

enum PRC_ProductLoadStatus 
{
  PRC_ProductLoadStatus_Unknown = 0,
  PRC_ProductLoadStatus_Error,
  PRC_ProductLoadStatus_NotLoaded,
  PRC_ProductLoadStatus_NotLoadable,
  PRC_ProductLoadStatus_Loaded
};

enum PRC_ExtendType 
{ 
  PRC_ExtendType_None      = 0,   // Discontinuous position.
  PRC_ExtendType_Ext1      = 2,   // Same as EPRCExtendTypeCInfinity.
  PRC_ExtendType_Ext2      = 4,   // Same as EPRCExtendTypeG1R for surface, and EPRCExtendTypeG1 for curve.
  PRC_ExtendType_G1        = 6,   // Continuous in direction but not magnitude of first derivative.
  PRC_ExtendType_G1R       = 8,   // Surface extended with a ruled surface that connects with G1-continuity.
  PRC_ExtendType_G1_G2     = 10,  // Extended by reflection, yielding a G2 continuous extension.
  PRC_ExtendType_CInfinity = 12   // Unlimited continuity.
};

// Note : this value is currently unused and should be set to PRC_KnotTypeUnspecified.
enum PRC_KnotType                     //  Defines knot vector type
{                   
  PRC_KnotType_UniformKnots,          // Uniform knot vector.
  PRC_KnotType_Unspecified,            // Unspecified knot type.
  PRC_KnotType_QuasiUniformKnots,      // Quasi-uniform knot vector.
  PRC_KnotType_PiecewiseBezierKnots    // Extrema with multiplicities of degree +1.
};          

enum PRC_BSplineSurfaceForm 
{
  PRC_BSplineSurfaceForm_Plane,            // Planar surface.
  PRC_BSplineSurfaceForm_Cylindrical,      // Cylindrical surface.
  PRC_BSplineSurfaceForm_Conical,          // Conical surface.
  PRC_BSplineSurfaceForm_Spherical,        // Spherical surface.
  PRC_BSplineSurfaceForm_Revolution,      // Surface of revolution.
  PRC_BSplineSurfaceForm_Ruled,            // Ruled surface.
  PRC_BSplineSurfaceForm_GeneralizedCone,  // Cone.
  PRC_BSplineSurfaceForm_Quadric,          // Quadric surface.
  PRC_BSplineSurfaceForm_LinearExtrusion,  // Surface of extrusion.
  PRC_BSplineSurfaceForm_Unspecified,      // Unspecified surface.
  PRC_BSplineSurfaceForm_Polynomial        // Polynomial surface.
};

// Note : this value is currently unused and should be set to PRC_BSplineCurveFormUnspecified.
enum PRC_BSplineCurveForm                 // Defines NURBS curve form.
{                
  PRC_BSplineCurveForm_Unspecified,        // Unspecified curve form.
  PRC_BSplineCurveForm_Polyline,          // Polygon.
  PRC_BSplineCurveForm_CircularArc,        // Circle arc.
  PRC_BSplineCurveForm_EllipticArc,        // Elliptical arc.
  PRC_BSplineCurveForm_ParabolicArc,      // Parabolic arc.
  PRC_BSplineCurveForm_HyperbolicArc      // Hyperbolic arc.
};          

enum PRC_TextureMappingType             // Defines how to retrieve mapping coordinates.
{            
  PRC_TextureMappingType_Unknown,        // Let the application choose.
  PRC_TextureMappingType_Stored,        // Use the mapping coordinates that are stored on a 3D tessellation object
  PRC_TextureMappingType_Parametric,    // Retrieve the UV coordinates on the surface as mapping coordinates
  PRC_TextureMappingType_Operator        // Use the defined Texture mapping operator to calculate mapping coordinates
};

enum PRC_TextureFunction           // Defines how to paint a texture on the surface being rendered.
{       
  PRC_TextureFunction_Unknown,     // Let the application choose.
  PRC_TextureFunction_Modulate,    // Combine lighting with texturing. This is the default value.
  PRC_TextureFunction_Replace,     // Replace the object color with texture color data.
  PRC_TextureFunction_Blend,       // Reserved for future use.
  PRC_TextureFunction_Decal         // Reserved for future use.
};

enum PRC_TextureMappingOperator            // The operator to use when computing mapping coordinates.
{          
  PRC_TextureMappingOperator_Unknown,       // Default value
  PRC_TextureMappingOperator_Planar,       // Reserved for future use
  PRC_TextureMappingOperator_Cylindrical,  // Reserved for future use
  PRC_TextureMappingOperator_Spherical,    // Reserved for future use
  PRC_TextureMappingOperator_Cubical        // Reserved for future use
};

enum PRC_TextureBlendParameter                  // Reserved for future use. Defines how to apply blending.
{                
  PRC_TextureBlendParameter_Unknown,            // Default value.
  PRC_TextureBlendParameter_Zero,                // Reserved for future use.
  PRC_TextureBlendParameter_One,                // Reserved for future use.
  PRC_TextureBlendParameter_SrcColor,            // Reserved for future use.
  PRC_TextureBlendParameter_OneMinusSrcColor,    // Reserved for future use.
  PRC_TextureBlendParameter_DstColor,            // Reserved for future use.
  PRC_TextureBlendParameter_OneMinusDstColor,    // Reserved for future use.
  PRC_TextureBlendParameter_SrcAlpha,            // Reserved for future use.
  PRC_TextureBlendParameter_OneMinusSrcAlpha,    // Reserved for future use.
  PRC_TextureBlendParameter_DstAlpha,            // Reserved for future use.
  PRC_TextureBlendParameter_OneMinusDstAlpha,    // Reserved for future use.
  PRC_TextureBlendParameter_SrcAlphaSaturate    // Reserved for future use.
};

enum PRC_TextureWrappingMode               // Defines repeating and clamping texture modes.
{           
  PRC_TextureWrappingMode_Unknown,        // Let the application choose.
  PRC_TextureWrappingMode_Repeat,          // Display the repeated texture on the surface.
  PRC_TextureWrappingMode_ClampToBorder,  // Clamp the texture to the border. Display the surface color along the texture limits.
  PRC_TextureWrappingMode_Clamp,          // Reserved for future use.
  PRC_TextureWrappingMode_ClampToEdge,    // Reserved for future use.
  PRC_TextureWrappingMode_MirroredRepeat  // Reserved for future use.
};

enum PRC_TextureAlphaTest           // Reserved for future use. Defines how to use a texture alpha test.
{          
  PRC_TextureAlphaTest_Unknown,      // Default value.
  PRC_TextureAlphaTest_Never,        // Reserved for future use.
  PRC_TextureAlphaTest_Less,        // Reserved for future use.
  PRC_TextureAlphaTest_Equal,        // Reserved for future use.
  PRC_TextureAlphaTest_Lequal,      // Reserved for future use.
  PRC_TextureAlphaTest_Greater,      // Reserved for future use.
  PRC_TextureAlphaTest_Notequal,    // Reserved for future use.
  PRC_TextureAlphaTest_Gequal,      // Reserved for future use.
  PRC_TextureAlphaTest_Always        // Reserved for future use.
};

enum PRC_MarkupType                 // Defines Markup types.
{               
  PRC_MarkupType_Unknown = 0,       // Unknown.
  PRC_MarkupType_Text,              // Text.
  PRC_MarkupType_Dimension,         // Dimension.
  PRC_MarkupType_Arrow,             // Arrow.
  PRC_MarkupType_Balloon,           // Balloon.
  PRC_MarkupType_CircleCenter,      // Circle Center.
  PRC_MarkupType_Coordinate,        // Coordinate.
  PRC_MarkupType_Date,              // Date.
  PRC_MarkupType_Fastener,          // Fastener.
  PRC_MarkupType_Gdt,               // Gdt.
  PRC_MarkupType_Locator,           // Locator.
  PRC_MarkupType_MeasurementPoint,  // Measurement Point.
  PRC_MarkupType_Roughness,         // Roughness.
  PRC_MarkupType_Welding,           // Welding.
  PRC_MarkupType_Table,             // Table.
  PRC_MarkupType_Other              // Other.
};

enum PRC_MarkupSubType                              // Defines Markup subtypes.
{                            
  PRC_MarkupSubType_Unknown = 0,                    // Unknown.
  PRC_MarkupSubType_EnumMax,                        

  PRC_MarkupSubType_Date_Ident = 1 ,                // Ident date.
  PRC_MarkupSubType_Date_EnumMax,                     

  PRC_MarkupSubType_Dimension_Distance = 1,         // Dimension distance.
  PRC_MarkupSubType_Dimension_Radius_Tangent,       // Dimension radius tangent.
  PRC_MarkupSubType_Dimension_Radius_Cylinder,      // Dimension radius cylinder.
  PRC_MarkupSubType_Dimension_Radius_Edge,          // Dimension radius edge.
  PRC_MarkupSubType_Dimension_Diameter,             // Dimension diameter.
  PRC_MarkupSubType_Dimension_Diameter_Tangent,     // Dimension diameter tangent.
  PRC_MarkupSubType_Dimension_Diameter_Cylinder,    // Dimension diameter cylinder.
  PRC_MarkupSubType_Dimension_Diameter_Edge,        // Dimension diameter edge.
  PRC_MarkupSubType_Dimension_Diameter_Cone,        // Dimension diameter cone.
  PRC_MarkupSubType_Dimension_Length,               // Dimension length.
  PRC_MarkupSubType_Dimension_Length_Curvilinear,   // Dimension length curvilinear.
  PRC_MarkupSubType_Dimension_Length_Circular,      // Dimension length circular.
  PRC_MarkupSubType_Dimension_Angle,                // Dimension angle.
  PRC_MarkupSubType_Dimension_EnumMax,              // Dimension 

  PRC_MarkupSubType_Gdt_Fcf = 1,                    // Gdt Fcf.
  PRC_MarkupSubType_Gdt_EnumMax,                    
  
  PRC_MarkupSubType_Welding_Line = 1,               // Welding line.
  PRC_MarkupSubType_Welding_EnumMax,                
 
  PRC_MarkupSubType_Other_Symbol_User = 1,          // Other symbol user.
  PRC_MarkupSubType_Other_EnumMax                   
};

enum PRC_CharSet                      // Defines character set.
{
  PRC_Charset_Unknown = -1,
  PRC_Charset_Roman = 0,
  PRC_Charset_Japanese,
  PRC_Charset_TraditionalChinese,
  PRC_Charset_Korean,
  PRC_Charset_Arabic,
  PRC_Charset_Hebrew,
  PRC_Charset_Greek,
  PRC_Charset_Cyrillic,
  PRC_Charset_RightLeft,
  PRC_Charset_Devanagari,
  PRC_Charset_Gurmukhi,
  PRC_Charset_Gujarati,
  PRC_Charset_Oriya,
  PRC_Charset_Bengali,
  PRC_Charset_Tamil,
  PRC_Charset_Telugu,
  PRC_Charset_Kannada,
  PRC_Charset_Malayalam,
  PRC_Charset_Sinhalese,
  PRC_Charset_Burmese,
  PRC_Charset_Khmer,
  PRC_Charset_Thai,
  PRC_Charset_Laotian,
  PRC_Charset_Georgian,
  PRC_Charset_Armenian,
  PRC_Charset_SimplifiedChinese,
  PRC_Charset_Tibetan,
  PRC_Charset_Mongolian,
  PRC_Charset_Geez,
  PRC_Charset_EastEuropeanRoman,
  PRC_Charset_Vietnamese,
  PRC_Charset_ExtendedArabic
};
...
struct PRC_RGBColor
{
  PRC_RGBColor(double r = 0.0, double g = 0.0, double b = 0.0) : red(r), green(g), blue(b) {}
  double red,green,blue;

  void serializeRgbColor(PRC_BitStream&);

  bool operator==(const PRC_RGBColor &c) const
  {
...
struct PRC_RGBAColor
{
  PRC_RGBAColor(double r=0.0, double g=0.0, double b=0.0, double a=1.0) : R(r), G(g), B(b), A(a) {}
  double R,G,B,A;
  ...
  bool operator==(const PRC_RGBAColor &c) const
  {
  ...
  friend PRC_RGBAColor operator * (const PRC_RGBAColor& a, const double d)  { return PRC_RGBAColor(a.R*d,a.G*d,a.B*d,a.A*d); }
  friend PRC_RGBAColor operator * (const double d, const PRC_RGBAColor& a)  { return PRC_RGBAColor(a.R*d,a.G*d,a.B*d,a.A*d); }

};

typedef std::map<PRC_RGBAColor,MLuint32> PRC_ColorMap;

struct PRC_RGBAColorWidth
{
  PRC_RGBAColorWidth(double r=0.0, double g=0.0, double b=0.0, double a=1.0, double w=1.0) : R(r), G(g), B(b), A(a), W(w) {}
  double R,G,B,A,W;

  bool operator==(const PRC_RGBAColorWidth &c) const
  {
...
typedef std::map<PRC_RGBAColorWidth,MLuint32> PRC_ColorWidthMap;
...
struct PRC_AttributeEntry
{
  PRC_AttributeEntry() : title_is_integer(false) {}
  PRC_AttributeEntry(MLuint32 integer) : title_is_integer(true)  { title_integer = integer; }
  PRC_AttributeEntry(const std::string &text) : title_is_integer(false) { title_text = text; }

  void serializeAttributeEntry(PRC_BitStream&) const;

  bool title_is_integer;
  std::string title_text;
  MLuint32 title_integer;
};

struct MLint32_Triple
{
  MLint32 x;
  MLint32 y;
  MLint32 z;
