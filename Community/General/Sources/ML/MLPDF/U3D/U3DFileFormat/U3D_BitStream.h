...
const MLint32 U3D_DATASIZEINCREMENT = 0x0000000F;
...
public:
  ...
  {
    U3DDataBlockFundamental symbol = (U3DDataBlockFundamental) uValue;
    swapBits8(symbol);
    bool escape = false;
    writeSymbol(U3DContext_Uncompressed8, symbol, escape);
  }
...
private:
  ...
  */
  void swapBits8(MLuint32 &rValue) { rValue = (U3D_Swap8[(rValue) & 0xf] << 4) | (U3D_Swap8[(rValue) >> 4]); }
  ...
  */
  ...
  U3DContextManager       _contextManager; // the context manager handles the updates to the histograms for the compression contexts.
  ...
  U3DDataVector           _data;           // the data section of the datablock to write.
  MLint32                 _dataPosition;   // the position currently to write in the datablock specified in 32 bit increments.
  U3DDataBlockFundamental _dataLocal;      // the local value of the data corresponding to dataposition
  U3DDataBlockFundamental _dataLocalNext;  // the 32 bits in data after dataLocal
