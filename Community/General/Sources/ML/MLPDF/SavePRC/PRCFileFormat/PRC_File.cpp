...
#define WriteUnsignedInteger( value ) out << (MLuint32)(value);
...
#define SerializeUserData PRC_UserData(0,0).write(out);
#define SerializeEmptyContentPRCBase PRC_ContentBase(PRC_TYPE_ROOT_PRCBase).serializeContentPRCBase(out);
...
#define SerializeUncompressedFiles  \
 { \
  const MLuint32 number_of_uncompressed_files = (MLuint32)uncompressed_files.size(); \
...
namespace prc {
  // Map [0,1] to [0,255]
  inline MLuint8 byte(double r) 
  {
    if(r < 0.0) r=0.0;
    else if(r > 1.0) r=1.0;
    MLuint8 a=(MLuint8)(256.0*r);
    if(a == 256) a=255;
    return a;
  }
}

void PRC_FileStructure::serializeFileStructureGlobals(PRC_BitStream &out)
{
  // even though this is technically not part of this section,
  // it is handled here for convenience
  const MLuint32 number_of_schema = 0;
  WriteUnsignedInteger (number_of_schema)

  WriteUnsignedInteger (PRC_TYPE_ASM_FileStructureGlobals)

  PRC_SingleAttribute sa((int32_t)PRC_VERSION);
  PRC_Attribute a("__PRC_RESERVED_ATTRIBUTE_PRCInternalVersion");
  a.addKey(sa);
  PRC_ContentBase cb(PRC_TYPE_ROOT_PRCBase);
  cb.addAttribute(a);
  ...
  const MLuint32 number_of_fonts = (MLuint32)font_keys_of_font.size();
  WriteUnsignedInteger (number_of_fonts)
  for (MLuint32 i=0;i<number_of_fonts;i++)
  {
    SerializeFontKeysSameFont (font_keys_of_font[i])
  }

  const MLuint32 number_of_colors = (MLuint32)colors.size();
  WriteUnsignedInteger (number_of_colors)
  for (MLuint32 i=0;i<number_of_colors;i++)
      SerializeRgbColor (colors[i])

  const MLuint32 number_of_pictures = (MLuint32)pictures.size();
  WriteUnsignedInteger (number_of_pictures)
  for (MLuint32 i=0;i<number_of_pictures;i++)
     SerializePicture (pictures[i])

  const MLuint32 number_of_texture_definitions = (MLuint32)texture_definitions.size();
  WriteUnsignedInteger (number_of_texture_definitions)
  for (MLuint32 i=0;i<number_of_texture_definitions;i++)
     SerializeTextureDefinition (texture_definitions[i])

  const MLuint32 number_of_materials = (MLuint32)materials.size();
  WriteUnsignedInteger (number_of_materials)
  for (MLuint32 i=0;i<number_of_materials;i++)
     SerializeMaterial (materials[i])

  // number of line patterns hard coded for now
  const MLuint32 number_of_line_patterns = 1;
  WriteUnsignedInteger (number_of_line_patterns)
  PRC_LinePattern().serializeLinePattern(out);

  const MLuint32 number_of_styles = (MLuint32)styles.size();
  WriteUnsignedInteger (number_of_styles)
  for (MLuint32 i=0;i<number_of_styles;i++)
     SerializeCategory1LineStyle (styles[i])

  const MLuint32 number_of_fill_patterns = 0;
  WriteUnsignedInteger (number_of_fill_patterns)

  const MLuint32 number_of_reference_coordinate_systems = (MLuint32)reference_coordinate_systems.size();
  WriteUnsignedInteger (number_of_reference_coordinate_systems)
  for (MLuint32 i=0;i<number_of_reference_coordinate_systems;i++)
     SerializeCoordinateSystem (reference_coordinate_systems[i])

  SerializeUserData
}

void PRC_FileStructure::serializeFileStructureTree(PRC_BitStream &out)
{
  WriteUnsignedInteger (PRC_TYPE_ASM_FileStructureTree)

  SerializeEmptyContentPRCBase

  const MLuint32 number_of_part_definitions = (MLuint32)part_definitions.size();
  ...
  SerializeEmptyContentPRCBase
  const MLuint32 next_available_index = createPRCUniqueIdentifier();
  WriteUnsignedInteger (next_available_index)
  const MLuint32 index_product_occurence = number_of_product_occurrences;  // Asymptote (PRCFile) specific - we write the root product last
  WriteUnsignedInteger (index_product_occurence)

  SerializeUserData
}

void PRC_FileStructure::serializeFileStructureTessellation(PRC_BitStream &out)
{
  WriteUnsignedInteger (PRC_TYPE_ASM_FileStructureTessellation)

  SerializeEmptyContentPRCBase
  const MLuint32 number_of_tessellations = (MLuint32)tessellations.size();
  WriteUnsignedInteger (number_of_tessellations)
  for (MLuint32 i=0;i<number_of_tessellations;i++)
    tessellations[i]->serializeBaseTessData(out);

  SerializeUserData
}

void PRC_FileStructure::serializeFileStructureGeometry(PRC_BitStream &out)
{
  WriteUnsignedInteger (PRC_TYPE_ASM_FileStructureGeometry)

  SerializeEmptyContentPRCBase
  const MLuint32 number_of_contexts = (MLuint32)contexts.size();
  WriteUnsignedInteger (number_of_contexts)
  for (MLuint32 i=0;i<number_of_contexts;i++)
    SerializeContextAndBodies (contexts[i])

  SerializeUserData
}

void PRC_FileStructure::serializeFileStructureExtraGeometry(PRC_BitStream &out)
{
...
void PRCFile::serializeModelFileData(PRC_BitStream &out)
{
  // even though this is technically not part of this section,
  // it is handled here for convenience
  const MLuint32 number_of_schema = 0;
  ...
  PRC_Attribute a("__PRC_RESERVED_ATTRIBUTE_PRCInternalVersion");
  a.addKey(sa);
  PRC_ContentBase cb(PRC_TYPE_ROOT_PRCBase,"PRC file");
  cb.addAttribute(a);
  cb.serializeContentPRCBase(out);

  SerializeUnit (unit)

  out << (MLuint32)1; // 1 product occurrence
  //UUID
  SerializeCompressedUniqueId( fileStructures[0]->file_structure_uuid )
  // index+1
  out << (MLuint32)fileStructures[0]->product_occurrences.size();
  // active
  out << true;
  out << (MLuint32)0; // index in model file

  SerializeUserData
}

void makeFileUUID(PRC_UniqueID& UUID)
{
  // make a UUID
  static MLuint32 count = 0;
  ++count;
  // the minimum requirement on UUIDs is that all must be unique in the file
  UUID.id0 = 0x33595341; // some constant
  UUID.id1 = (MLuint32)time(NULL); // the time
  UUID.id2 = count;
...
void makeAppUUID(PRC_UniqueID& UUID)
{
...
MLuint32 PRCUncompressedFile::getSize() const
{
...
MLuint32 PRCStartHeader::getStartHeaderSize() const
{
  return 3+(2+2*4)*sizeof(MLuint32);
}


void PRC_FileStructure::write(ostream &out)
{
...
void PRC_FileStructure::prepare()
{
  MLuint32 size = 0;
  size += getStartHeaderSize();
  size += sizeof(MLuint32);
  for(PRCUncompressedFileList::const_iterator it = uncompressed_files.begin(); it != uncompressed_files.end(); it++)
    size += (*it)->getSize();
  sizes[0]=size;
...
MLuint32 PRC_FileStructure::getSize()
{
  MLuint32 size = 0;
...
void PRCFileStructureInformation::write(ostream &out)
{
  ...
  for(MLuint32 i = 0; i < number_of_offsets; ++i)
  {
    WriteUncompressedUnsignedInteger (offsets[i])
  }
}

MLuint32 PRCFileStructureInformation::getSize()
{
  return (4+2+number_of_offsets)*sizeof(MLuint32);
}
...
MLuint32 PRCHeader::getSize()
{
  MLuint32 size = getStartHeaderSize() + sizeof(MLuint32);
  for(MLuint32 i = 0; i < number_of_file_structures; ++i)
    size += fileStructureInformation[i].getSize();
  size += 3*sizeof(MLuint32);
  for(PRCUncompressedFileList::const_iterator it = uncompressed_files.begin(); it != uncompressed_files.end(); it++)
    size += (*it)->getSize();
  return size;
}

void PRCFile::doGroup(PRCgroup& group)
{
    const string& name = group.name;

    PRC_ProductOccurrence*& product_occurrence        = group.product_occurrence;
    PRC_ProductOccurrence*& parent_product_occurrence = group.parent_product_occurrence;
    PRC_PartDefinition*& part_definition              = group.part_definition;
    PRC_PartDefinition*& parent_part_definition       = group.parent_part_definition;
    ...
    if(group.options.tess)
    {
      if(!group.lines.empty())
      {
        for(PRCtesslineMap::const_iterator wit=group.lines.begin(); wit!=group.lines.end(); wit++)
        {
          ...
          const PRC_RGBColor &color = lines.front().color;
          ...
          map<PRC_Vector3D,MLuint32> points;
          PRC_3DWireTesselation *tess = new PRC_3DWireTesselation();
          if(!same_color)
          {
          ...
          for(PRCtesslineList::const_iterator lit=lines.begin(); lit!=lines.end(); lit++)
          {
            tess->wire_indexes.push_back((MLuint32)lit->point.size());
            for(MLuint32 i=0; i<lit->point.size(); i++)
            {
              map<PRC_Vector3D,MLuint32>::iterator pPoint = points.find(lit->point[i]);
              if(pPoint!=points.end())
                tess->wire_indexes.push_back(pPoint->second);
              else
              {
                const MLuint32 point_index = (MLuint32)tess->coordinates.size();
          ...
          const MLuint32 tess_index = add3DWireTess(tess);
          PRC_PolyWire *polyWire = new PRC_PolyWire();
          polyWire->index_tessellation = tess_index;
          if(same_color)
            polyWire->index_of_line_style = addColourWidth(PRC_RGBAColor(color.red,color.green,color.blue),wit->first);
          else
            polyWire->index_of_line_style = addColourWidth(PRC_RGBAColor(1,1,1),wit->first);
...
//    make rectangles pairs of triangles in a tesselation
      ...
      {
        bool same_color = true;
        const MLuint32 &style = group.rectangles.front().style;
        ...
        map<PRC_Vector3D,MLuint32> points;
        PRC_3DTesselation *tess = new PRC_3DTesselation();
        tess->crease_angle = group.options.crease_angle;
        PRC_TesselationFace *tessFace = new PRC_TesselationFace();
        tessFace->used_entities_flag=PRC_FACETESSDATA_Triangle;
        MLuint32 triangles = 0;
        for(PRCtessrectangleList::const_iterator rit=group.rectangles.begin(); rit!=group.rectangles.end(); rit++)
        {
          const bool degenerate = (rit->vertices[0]==rit->vertices[1]);
          MLuint32 vertex_indices[4];
          for(size_t i = (degenerate?1:0); i < 4; ++i)
          {
            map<PRC_Vector3D,MLuint32>::const_iterator pPoint = points.find(rit->vertices[i]);
            if(pPoint!=points.end())
              vertex_indices[i] =  pPoint->second;
            else
            {
              points.insert(make_pair(rit->vertices[i],(vertex_indices[i] = (MLuint32)tess->coordinates.size())));
        ...
        const MLuint32 tess_index = add3DTess(tess);
        PRC_PolyBoundaryRepresentationModel *polyBrepModel = new PRC_PolyBoundaryRepresentationModel();
    ...
    if(!group.quads.empty())
    {
      ...
      {
        const PRC_RGBAColor* C = qit->colours;
      ...
      const PRC_RGBAColor& colour = group.quads.front().colours[0];
      for(PRCtessquadList::const_iterator qit=group.quads.begin(); qit!=group.quads.end(); qit++)
      {
        const PRC_RGBAColor* C = qit->colours;
      ...
      for(PRCtessquadList::const_iterator qit=group.quads.begin(); qit!=group.quads.end(); qit++)
      {
        ...
        for(size_t i = (degenerate?1:0); i < 4; ++i)
        {
          map<PRC_Vector3D,MLuint32>::const_iterator pPoint = points.find(qit->vertices[i]);
          if(pPoint!=points.end())
            vertex_indices[i] =  pPoint->second;
          else
          {
            points.insert(make_pair(qit->vertices[i],(vertex_indices[i] = (MLuint32)tess->coordinates.size())));
    ...
    if(!group.points.empty())
    {
      ...
      {
        PRC_PointSet *pointset = new PRC_PointSet();
    ...
    {
      for(std::vector<PRC_PointSet*>::iterator pit=group.pointsets.begin(); pit!=group.pointsets.end(); pit++)
      {
    ...
    {
      for(std::vector<PRC_PolyBoundaryRepresentationModel*>::iterator pit=group.polymodels.begin(); pit!=group.polymodels.end(); pit++)
      {
    ...
    {
      for(std::vector<PRC_PolyWire*>::iterator pit=group.polywires.begin(); pit!=group.polywires.end(); pit++)
      {
    ...
    {
      PRC_TopologyContext *wireContext = NULL;
      const MLuint32 context_index = getTopoContext(wireContext);
      for(PRCwireList::iterator wit=group.wires.begin(); wit!=group.wires.end(); wit++)
      {
        PRC_WireEdge *wireEdge = new PRC_WireEdge;
        wireEdge->curve_3d = wit->curve;
        PRC_SingleWireBody *wireBody = new PRC_SingleWireBody;
        wireBody->setWireEdge(wireEdge);
        const MLuint32 wire_body_index = wireContext->addSingleWireBody(wireBody);
        PRC_Wire *wire = new PRC_Wire();
    ...
    {
      bool same_color = true;
      const MLuint32 style = faces.front().style;
      ...
      PRC_TopologyContext *context = NULL;
      const MLuint32 context_index = getTopoContext(context);
   ...
   // context->smallest_thickness = group.options.granularity;
      ...
      for(PRCfaceList::iterator fit=faces.begin(); fit!=faces.end(); fit++)
      {
           ...
           (fit->transparent && !group.options.no_break))
        {
          ...
          PRC_Connex *connex = new PRC_Connex;
          connex->addShell(shell);
          PRC_BoundaryRepresentationData *body = new PRC_BoundaryRepresentationData;
          body->addConnex(connex);
          const MLuint32 body_index = context->addBrepData(body);

          PRC_BoundaryRepresentationModel *brepmodel = new PRC_BoundaryRepresentationModel();
    ...
    {
      bool same_color = true;
      const MLuint32 style = compfaces.front().style;
      ...
      PRC_TopologyContext *context = NULL;
      const MLuint32 context_index = getTopoContext(context);
      PRC_CompressedBoundaryRepresentationData *body = new PRC_CompressedBoundaryRepresentationData;
      ...
      for(PRCcompfaceList::const_iterator fit=compfaces.begin(); fit!=compfaces.end(); fit++)
      {
           ...
           (fit->transparent && !group.options.no_break))
        {
          ...
          const MLuint32 body_index = context->addCompressedBrepData(body);
    ...
    for(PRC_RepresentationItemList::const_iterator it=part_definition->representation_item.begin(); it!=part_definition->representation_item.end(); it++)
    {
      if (!(*it)->name.empty())
      {
        nonamedparts = false;
    ...
    if (parent_part_definition && product_occurrence->index_son_occurrence.empty() &&
        ...
        ( !group.transform  || part_definition->representation_item.front()->index_local_coordinate_system == MLuint32_MAX) )
    {
      if(part_definition->representation_item.front()->name.empty() )
        part_definition->representation_item.front()->name = name;
      if(part_definition->representation_item.front()->index_local_coordinate_system == MLuint32_MAX)
      ...
      !group.options.do_break && nonamedparts)
    {
      ...
      for(PRC_RepresentationItemList::iterator it=part_definition->representation_item.begin(); it!=part_definition->representation_item.end(); it++)
      {
...
std::string PRCFile::calculate_unique_name(const PRC_ContentBase *prc_entity,const PRC_ContentBase *prc_occurence)
{
  std::stringstream ss (std::stringstream::in | std::stringstream::out);
  MLuint8 *serialization_buffer = NULL;
  PRC_BitStream serialization(serialization_buffer,0u);
  const PRC_FileStructure *pfile_structure = fileStructures[0];
  const PRC_UniqueID& uuid = pfile_structure->file_structure_uuid;
...
// serialization_buffer = Flush serialization (serialization) 
  {
    const MLuint32 size_serialization = serialization.getSize();
...
// SerializeCompressedUniqueId (file_structure)
    serialization << uuid.id0 << uuid.id1 << uuid.id2 << uuid.id3;
// WriteUnsignedInteger (type)
    serialization << (MLuint32)PRC_TYPE_ASM_ProductOccurence;
// WriteUnsignedInteger (unique_identifier)
    serialization << prc_occurence->getPRCID();
  }
  ss << (prc_entity->name.empty()?"_node_":prc_entity->name) << '.';
  const MLuint32 size_serialization = serialization.getSize();
  for(size_t j=0; j<size_serialization; j++)
    ss << hex << setfill('0') << setw(2) << (MLuint32)(serialization_buffer[j]);
...
bool PRCFile::finish()
{
  ...
  for(MLuint32 i = 0; i < number_of_file_structures; ++i)
  {
    ...
    header.fileStructureInformation[i].offsets = new MLuint32[6];
  }

  header.minimal_version_for_read = PRC_VERSION;
  header.authoring_version = PRC_VERSION;
  ...
  MLuint32 currentOffset = header.getSize();
...
MLuint32 PRCFile::getSize()
{
  MLuint32 size = header.getSize();
...
MLuint32 PRC_FileStructure::addPicture(PRC_PictureDataFormat format, MLuint32 size, const MLuint8 *p, MLuint32 width, MLuint32 height, string name)
{
  MLuint8 *data = NULL;
  MLuint32 components=0;
  PRC_Picture picture(name);
  if(size==0 || p==NULL)
    { cerr << "image not set" << endl; return MLuint32_MAX; }
  PRCUncompressedFile* uncompressed_file = new PRCUncompressedFile;
  if(format==PRC_PictureDataFormat_PNG || format==PRC_PictureDataFormat_JPG)
  {
    data = new MLuint8[size];
    memcpy(data, p, size);
    ...
    picture.uncompressed_file_index = (MLuint32)uncompressed_files.size()-1;
    ...
    return (MLuint32)pictures.size()-1;
  }

  switch(format)
  {
    case PRC_PictureDataFormat_Bitmap_RGB:
      components = 3; break;
    case PRC_PictureDataFormat_Bitmap_RGBAlpha:
      components = 4; break;
    case PRC_PictureDataFormat_Bitmap_Grey:
      components = 1; break;
    case PRC_PictureDataFormat_Bitmap_GreyAlpha:
      components = 2; break;
    default:
      { cerr << "unknown picture format" << endl; return MLuint32_MAX; }
  }
      if(width==0 || height==0)
        { cerr << "width or height parameter not set" << endl; return MLuint32_MAX; }
      if (size < width*height*components)
        { cerr << "image too small" << endl; return MLuint32_MAX; }

      {
        MLuint32 compressedDataSize = 0;
        ...
        if(deflateInit(&strm,Z_DEFAULT_COMPRESSION) != Z_OK)
          { cerr << "Compression initialization failed" << endl; return MLuint32_MAX; }
        ...
        while((code = deflate(&strm,Z_FINISH)) == Z_OK)
        {
          ...
          compressedData = (MLuint8*) realloc(compressedData,CHUNK*chunks);
        ...
        if(code != Z_STREAM_END)
        {
          deflateEnd(&strm);
          free(compressedData);
          { cerr << "Compression error" << endl; return MLuint32_MAX; }
        }

        deflateEnd(&strm);
        size = compressedDataSize;
        data = new MLuint8[compressedDataSize];
...
MLuint32 PRC_FileStructure::addTextureDefinition(PRC_TextureDefinition*& pTextureDefinition)
{
  texture_definitions.push_back(pTextureDefinition);
  pTextureDefinition = NULL;
  return (MLuint32)texture_definitions.size()-1;
}

MLuint32 PRC_FileStructure::addRgbColor(const PRC_RGBColor &color)
{
  colors.push_back(color);
  return (MLuint32)(3*(colors.size()-1));
}

MLuint32 PRC_FileStructure::addRgbColorUnique(const PRC_RGBColor &color)
{
  for(MLuint32 i = 0; i < colors.size(); ++i)
  {
    if(colors[i] == color)
      return 3*i;
  }
...
MLuint32 PRCFile::addColor(const PRC_RGBColor &color)
{
...
//  color_index = addRgbColorUnique(color);
  const MLuint32 color_index = fileStructures[0]->addRgbColor(color);
  colorMap.insert(make_pair(color,color_index));
  return color_index;
}

MLuint32 PRCFile::addColour(const PRC_RGBAColor &colour)
{
  PRC_ColorMap::const_iterator pColour = colourMap.find(colour);
  if(pColour!=colourMap.end())
    return pColour->second;
  const MLuint32 color_index = addColor(PRC_RGBColor(colour.R, colour.G, colour.B));
  PRC_Style *style = new PRC_Style();
  ...
  style->transparency = (MLuint8)(colour.A * 256);
  style->additional = 0;
  const MLuint32 style_index = fileStructures[0]->addStyle(style);
  colourMap.insert(make_pair(colour,style_index));
  return style_index;
}

MLuint32 PRCFile::addColourWidth(const PRC_RGBAColor &colour, double width)
{
  PRC_RGBAColorWidth colourwidth(colour.R, colour.G, colour.B, colour.A, width);
  PRC_ColorWidthMap::const_iterator pColour = colourwidthMap.find(colourwidth);
...
MLuint32 PRCFile::addTransform(PRC_GeneralTransformation3D*& transform)
{
  if(!transform)
    return MLuint32_MAX;
  ...
  PRC_CoordinateSystem *coordinateSystem = new PRC_CoordinateSystem();
      ...
      transform->M(3,0)==0 && transform->M(3,1)==0 && transform->M(3,2)==0 && transform->M(3,3)==1 )
  {
    ...
    PRC_CartesianTransformation3D *carttransform = new PRC_CartesianTransformation3D;
...
//    carttransform->behaviour = PRC_TRANSFORMATION_Identity;
  ...
  const MLuint32 coordinate_system_index = fileStructures[0]->addCoordinateSystem(coordinateSystem);
...
MLuint32 PRCFile::addTransform(const double* t)
{
  if(!t)
    return MLuint32_MAX;
  PRC_GeneralTransformation3D* transform = new PRC_GeneralTransformation3D(t);
  return addTransform(transform);
}

MLuint32 PRCFile::addTransform(const double origin[3], const double x_axis[3], const double y_axis[3], double scale)
{
  PRC_CartesianTransformation3D* transform = new PRC_CartesianTransformation3D(origin, x_axis, y_axis, scale);
...
MLuint32 PRCFile::addMaterial(const PRCmaterial& m)
{
  MLuint32 material_index = MLuint32_MAX;
...
{
  PRC_MaterialGeneric *materialGeneric = new PRC_MaterialGeneric();
  const PRC_RGBColor ambient(m.ambient.R, m.ambient.G, m.ambient.B);
  materialGeneric->ambient = addColor(ambient);
    const PRC_RGBColor diffuse(m.diffuse.R, m.diffuse.G, m.diffuse.B);
  materialGeneric->diffuse = addColor(diffuse);
    const PRC_RGBColor emissive(m.emissive.R, m.emissive.G, m.emissive.B);

  materialGeneric->emissive = addColor(emissive);
    const PRC_RGBColor specular(m.specular.R, m.specular.G, m.specular.B);
  ...
  MLuint32 color_material_index = MLuint32_MAX;
  if(m.picture_data!=NULL)
  {
    MLuint32 picture_index = MLuint32_MAX;
    ...
    {
      PRC_TextureDefinition *TextureDefinition = new PRC_TextureDefinition;
      if (m.picture_size==216688 && m.picture_format==PRC_PictureDataFormat_JPG)
        TextureDefinition->texture_mapping_attribute=PRC_TEXTURE_MAPPING_OPACITY;
      TextureDefinition->picture_index = picture_index;
      TextureDefinition->texture_function = m.picture_replace ? PRC_TextureFunction_Replace : PRC_TextureFunction_Modulate;
      TextureDefinition->texture_wrapping_mode_S = m.picture_repeat ? PRC_TextureWrappingMode_Repeat : PRC_TextureWrappingMode_ClampToEdge;
      TextureDefinition->texture_wrapping_mode_T = m.picture_repeat ? PRC_TextureWrappingMode_Repeat : PRC_TextureWrappingMode_ClampToEdge;
      TextureDefinition->texture_mapping_attribute_components = (m.picture_format==PRC_PictureDataFormat_Bitmap_RGB || m.picture_format==PRC_PictureDataFormat_JPG) ? PRC_TEXTURE_MAPPING_COMPONENTS_RGB : PRC_TEXTURE_MAPPING_COMPONENTS_RGBA;
    ...
    MLuint32 texture_application_index = MLuint32_MAX;
    ...
    {
      PRC_TextureApplication *TextureApplication = new PRC_TextureApplication;
  ...
  MLuint32 style_index = MLuint32_MAX;
  ...
  else
  {
    ...
    Style->transparency = (MLuint8)(m.alpha * 256);
    Style->additional = 0;
...
void PRCFile::begingroup(const char *name, PRCoptions *options,
                          const double* t)
{
  ...
  if(t&&!isid(t))
    group.transform = new PRC_GeneralTransformation3D(t);
  group.product_occurrence = new PRC_ProductOccurrence(name);
...
#define ADDCOMPFACE                                       \
  ...
  PRC_CompressedFace *compface = new PRC_CompressedFace;    \
  face.face = compface;                                   \
  face.transparent = m.alpha < 1.0;                       \
  face.style = addMaterial(m);

void PRCFile::addPoint(const double P[3], const PRC_RGBAColor &c, double w)
{
  PRCgroup &group = findGroup();
  group.points[addColourWidth(c,w)].push_back(PRC_Vector3D(P[0],P[1],P[2]));
}

void PRCFile::addPoints(MLuint32 n, const double P[][3], const PRC_RGBAColor &c, double w)
{
  if(n==0 || P==NULL)
     return;
  PRCgroup &group = findGroup();
  ...
  for(MLuint32 i=0; i<n; i++)
    pointset->point.push_back(PRC_Vector3D(P[i][0],P[i][1],P[i][2]));
}

void PRCFile::useMesh(MLuint32 tess_index, MLuint32 style_index, const double origin[3], const double x_axis[3], const double y_axis[3], double scale)
{
  PRCgroup &group = findGroup();
  PRC_PolyBoundaryRepresentationModel *polyBrepModel = new PRC_PolyBoundaryRepresentationModel();
...
void PRCFile::useMesh(MLuint32 tess_index, MLuint32 style_index, const double* t)
{
  PRCgroup &group = findGroup();
  PRC_PolyBoundaryRepresentationModel *polyBrepModel = new PRC_PolyBoundaryRepresentationModel();
...
void PRCFile::useLines(MLuint32 tess_index, MLuint32 style_index, const double origin[3], const double x_axis[3], const double y_axis[3], double scale)
{
  PRCgroup &group = findGroup();
  PRC_PolyWire *polyWire = new PRC_PolyWire();
...
void PRCFile::useLines(MLuint32 tess_index, MLuint32 style_index, const double* t)
{
  PRCgroup &group = findGroup();
  PRC_PolyWire *polyWire = new PRC_PolyWire();
...
void PRCFile::addTriangles(MLuint32 nP, const double P[][3], MLuint32 nI, const MLuint32 PI[][3], const PRCmaterial &m,
 ...
 MLuint32 nM, const PRCmaterial M[], const MLuint32 MI[], double ca)
{
  if(nP==0 || P==NULL || nI==0 || PI==NULL)
     return;
  const MLuint32 tess_index = createTriangleMesh(nP, P, nI, PI, m, nN, N, NI, nT, T, TI, nC, C, CI, nM, M, MI, ca);
  useMesh(tess_index,MLuint32_MAX);
}

MLuint32 PRCFile::createTriangleMesh(MLuint32 nP, const double P[][3], MLuint32 nI, const MLuint32 PI[][3], const MLuint32 style_index,
 MLuint32 nN, const double N[][3],  const MLuint32 NI[][3],
 MLuint32 nT, const double T[][2],  const MLuint32 TI[][3],
 MLuint32 nC, const PRC_RGBAColor C[], const MLuint32 CI[][3],
 MLuint32 nS, const MLuint32 S[],   const MLuint32 SI[], double ca)
{
  ...
  else if (style_index != MLuint32_MAX )
  {
...
void PRCFile::addQuads(MLuint32 nP, const double P[][3], MLuint32 nI, const MLuint32 PI[][4], const PRCmaterial &m,
 ...
 MLuint32 nM, const PRCmaterial M[], const MLuint32 MI[], double ca)
{
  if(nP==0 || P==NULL || nI==0 || PI==NULL)
     return;
  const MLuint32 tess_index = createQuadMesh(nP, P, nI, PI, m, nN, N, NI, nT, T, TI, nC, C, CI, nM, M, MI, ca);
  useMesh(tess_index,MLuint32_MAX);
}

MLuint32 PRCFile::createQuadMesh(MLuint32 nP, const double P[][3], MLuint32 nI, const MLuint32 PI[][4], MLuint32 style_index,
 MLuint32 nN, const double N[][3],   const MLuint32 NI[][4],
 MLuint32 nT, const double T[][2],   const MLuint32 TI[][4],
 MLuint32 nC, const PRC_RGBAColor C[],  const MLuint32 CI[][4],
 MLuint32 nS, const MLuint32 S[],    const MLuint32 SI[], double ca)
...
void PRCFile::addQuad(const double P[][3], const PRC_RGBAColor C[])
{
  PRCgroup &group = findGroup();
...
/*
void PRCFile::addTriangle(const double P[][3], const double T[][2], MLuint32 style_index)
{
  PRCgroup &group = findGroup();
...
void PRCFile::addLines(MLuint32 nP, const double P[][3], MLuint32 nI, const MLuint32 PI[],
 const PRC_RGBAColor& c, double w,
 bool segment_color, MLuint32 nC, const PRC_RGBAColor C[], MLuint32 nCI, const MLuint32 CI[])
{
  if(nP==0 || P==NULL || nI==0 || PI==NULL)
    return;
  const MLuint32 tess_index = createLines(nP, P, nI, PI, segment_color, nC, C, nCI, CI);
  useLines(tess_index, c, w);
}

MLuint32 PRCFile::createLines(MLuint32 nP, const double P[][3], MLuint32 nI, const MLuint32 PI[],
 bool segment_color, MLuint32 nC, const PRC_RGBAColor C[], MLuint32 nCI, const MLuint32 CI[])
{
  if(nP==0 || P==NULL || nI==0 || PI==NULL)
    return MLuint32_MAX;
  ...
  for(MLuint32 i=0; i<nI;)
  {
    tess->wire_indexes.push_back(PI[i]);
    const MLuint32 ni = i+PI[i]+1;
    for(i++; i<ni; i++)
...
void PRCFile::addLine(MLuint32 n, const double P[][3], const PRC_RGBAColor &c, double w)
{
  PRCgroup &group = findGroup();
  if(group.options.tess)
  {
    ...
    line.color.blue  = c.B;
    for(MLuint32 i=0; i<n; i++)
      line.point.push_back(PRC_Vector3D(P[i][0],P[i][1],P[i][2]));
  }
  else
  {
    ADDWIRE(PRC_PolyLine)
    curve->point.resize(n);
    for(MLuint32 i=0; i<n; i++)
     curve->point[i].Set(P[i][0],P[i][1],P[i][2]);
    curve->interval.min = 0;
    curve->interval.max = (double)(curve->point.size()-1);
  }
}

void PRCFile::addBezierCurve(MLuint32 n, const double cP[][3],
                              const PRC_RGBAColor &c)
{
  ADDWIRE(PRC_NURBSCurve)
  ...
  for(size_t i = 1; i < 3+NUMBER_OF_POINTS; ++i)
    curve->knot[i] = (double)((i+2)/3);                                  // Integer division is intentional
  curve->knot[3+NUMBER_OF_POINTS] = (double)((3+NUMBER_OF_POINTS+1)/3);  // Integer division is intentional
}

void PRCFile::addCurve(MLuint32 d, MLuint32 n, const double cP[][3], const double *k, const PRC_RGBAColor &c, const double w[])
{
  ADDWIRE(PRC_NURBSCurve)
  ...
  for(MLuint32 i = 0; i < d+n+1; i++)
    curve->knot[i] = k[i];
}

void PRCFile::addRectangle(const double P[][3], const PRCmaterial &m)
{
  PRCgroup &group = findGroup();
  if(group.options.tess)
  {
...
void PRCFile::addPatch(const double cP[][3], const PRCmaterial &m)
{
  PRCgroup &group = findGroup();
...
void PRCFile::addSurface(MLuint32 dU, MLuint32 dV, MLuint32 nU, MLuint32 nV,
                          ...
                          const double w[])
{
  ...
  for(size_t i = 0; i < nU*nV; i++)
    if(w)
      surface->control_point[i]=PRC_ControlPoint(cP[i][0]*w[i],cP[i][1]*w[i],cP[i][2]*w[i],w[i]);
    else
      surface->control_point[i]=PRC_ControlPoint(cP[i][0],cP[i][1],cP[i][2]);
...
#define SETTRANSF \
  if(t&&!isid(t))                                                             \
    face.transform = new PRC_GeneralTransformation3D(t);                       \
  ...
  if(surface->origin!=PRC_Vector3D(0.0,0.0,0.0))                                     \
    surface->behaviour = surface->behaviour | PRC_TRANSFORMATION_Translate;   \
  if(surface->x_axis!=PRC_Vector3D(1.0,0.0,0.0)||surface->y_axis!=PRC_Vector3D(0.0,1.0,0.0)) \
...
void PRCFile::addTube(MLuint32 n, const double cP[][3], const double oP[][3], bool straight, const PRCmaterial &m, PRCFACETRANSFORM)
{
  ADDFACE(PRC_Blend01)
  SETTRANSF
  if(straight)
  {
    PRC_PolyLine *center_curve = new PRC_PolyLine;
    center_curve->point.resize(n);
    for(MLuint32 i=0; i<n; i++)
      center_curve->point[i].Set(cP[i][0],cP[i][1],cP[i][2]);
    center_curve->interval.min = 0;
    center_curve->interval.max = (double)(center_curve->point.size()-1);
    surface->center_curve = center_curve;

    PRC_PolyLine *origin_curve = new PRC_PolyLine;
    origin_curve->point.resize(n);
    for(MLuint32 i=0; i<n; i++)
      origin_curve->point[i].Set(oP[i][0],oP[i][1],oP[i][2]);
    origin_curve->interval.min = 0;
    origin_curve->interval.max = (double)(origin_curve->point.size()-1);
  ...
  {
    PRC_NURBSCurve *center_curve = new PRC_NURBSCurve;
    center_curve->is_rational = false;
    center_curve->degree = 3;
    const MLuint32 CENTER_NUMBER_OF_POINTS = n;
    center_curve->control_point.resize(CENTER_NUMBER_OF_POINTS);
    for(MLuint32 i = 0; i < CENTER_NUMBER_OF_POINTS; ++i)
    ...
    for(MLuint32 i = 1; i < 3+CENTER_NUMBER_OF_POINTS; ++i)
    ...
    PRC_NURBSCurve *origin_curve = new PRC_NURBSCurve;
    origin_curve->is_rational = false;
    origin_curve->degree = 3;
    const MLuint32 ORIGIN_NUMBER_OF_POINTS = n;
    origin_curve->control_point.resize(ORIGIN_NUMBER_OF_POINTS);
    for(MLuint32 i = 0; i < ORIGIN_NUMBER_OF_POINTS; ++i)
    ...
    for(size_t i = 1; i < 3+ORIGIN_NUMBER_OF_POINTS; ++i)
      origin_curve->knot[i] = (double)((i+2)/3);                                      // Integer division is intentional
    origin_curve->knot[3+ORIGIN_NUMBER_OF_POINTS] = (3+ORIGIN_NUMBER_OF_POINTS+1)/3;  // Integer division is intentional
    surface->origin_curve = origin_curve;
...
void PRCFile::addHemisphere(double radius, const PRCmaterial &m, PRCFACETRANSFORM)
{
  ADDFACE(PRC_Sphere)
  SETTRANSF
  surface->uv_domain.min.x = 0;
  surface->uv_domain.max.x = 2*M_PI;
  surface->uv_domain.min.y = 0;
  surface->uv_domain.max.y = 0.5*M_PI;
  surface->radius = radius;
}

void PRCFile::addSphere(double radius, const PRCmaterial &m, PRCFACETRANSFORM)
{
  ADDFACE(PRC_Sphere)
  SETTRANSF
  surface->uv_domain.min.x = 0;
  surface->uv_domain.max.x = 2*M_PI;
  surface->uv_domain.min.y =-0.5*M_PI;
  surface->uv_domain.max.y = 0.5*M_PI;
  surface->radius = radius;
}

void PRCFile::addDisk(double radius, const PRCmaterial &m, PRCFACETRANSFORM)
{
  ADDFACE(PRC_Ruled)
  SETTRANSF
  PRC_Circle *first_curve = new PRC_Circle;
  first_curve->radius = radius;
  surface->first_curve = first_curve;
  PRC_Circle *second_curve = new PRC_Circle;
...
void PRCFile::addCylinder(double radius, double height, const PRCmaterial &m, PRCFACETRANSFORM)
{
  ADDFACE(PRC_Cylinder)
  SETTRANSF
  surface->uv_domain.min.x = 0;
  surface->uv_domain.max.x = 2*M_PI;
...
void PRCFile::addCone(double radius, double height, const PRCmaterial &m, PRCFACETRANSFORM)
{
  ADDFACE(PRC_Cone)
  SETTRANSF
  surface->uv_domain.min.x = 0;
...
void PRCFile::addTorus(double major_radius, double minor_radius, double angle1, double angle2, const PRCmaterial &m, PRCFACETRANSFORM)
{
  ADDFACE(PRC_Torus)
  SETTRANSF
...
MLuint32 PRC_FileStructure::addMaterialGeneric(PRC_MaterialGeneric*& pMaterialGeneric)
{
  materials.push_back(pMaterialGeneric);
  pMaterialGeneric = NULL;
  return (MLuint32)materials.size()-1;
}

MLuint32 PRC_FileStructure::addTextureApplication(PRC_TextureApplication*& pTextureApplication)
{
...
MLuint32 PRC_FileStructure::addStyle(PRC_Style*& pStyle)
{
  styles.push_back(pStyle);
  pStyle = NULL;
  return (MLuint32)styles.size()-1;
}

MLuint32 PRC_FileStructure::addPartDefinition(PRC_PartDefinition*& pPartDefinition)
{
  part_definitions.push_back(pPartDefinition);
  pPartDefinition = NULL;
  return (MLuint32)part_definitions.size()-1;
}

MLuint32 PRC_FileStructure::addProductOccurrence(PRC_ProductOccurrence*& pProductOccurrence)
{
  product_occurrences.push_back(pProductOccurrence);
  pProductOccurrence = NULL;
  return (MLuint32)product_occurrences.size()-1;
}

MLuint32 PRC_FileStructure::addTopoContext(PRC_TopologyContext*& pTopoContext)
{
  contexts.push_back(pTopoContext);
  pTopoContext = NULL;
  return (MLuint32)contexts.size()-1;
}

MLuint32 PRC_FileStructure::getTopoContext(PRC_TopologyContext*& pTopoContext)
{
  pTopoContext = new PRC_TopologyContext;
  contexts.push_back(pTopoContext);
  return (MLuint32)contexts.size()-1;
}

MLuint32 PRC_FileStructure::add3DTess(PRC_3DTesselation*& p3DTess)
{
  tessellations.push_back(p3DTess);
  p3DTess = NULL;
  return (MLuint32)tessellations.size()-1;
}

MLuint32 PRC_FileStructure::add3DWireTess(PRC_3DWireTesselation*& p3DWireTess)
{
...
/*
MLuint32 PRC_FileStructure::addMarkupTess(PRC_MarkupTesselation*& pMarkupTess)
{
...
MLuint32 PRC_FileStructure::addMarkup(PRC_Markup*& pMarkup)
{
...
MLuint32 PRC_FileStructure::addAnnotationItem(PRC_AnnotationItem*& pAnnotationItem)
{
...
*/
MLuint32 PRC_FileStructure::addCoordinateSystem(PRC_CoordinateSystem*& pCoordinateSystem)
{
  reference_coordinate_systems.push_back(pCoordinateSystem);
  pCoordinateSystem = NULL;
  return (MLuint32)reference_coordinate_systems.size()-1;
}

MLuint32 PRC_FileStructure::addCoordinateSystemUnique(PRC_CoordinateSystem*& pCoordinateSystem)
{
  for(MLuint32 i = 0; i < reference_coordinate_systems.size(); ++i)
  {
